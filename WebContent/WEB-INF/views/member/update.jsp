...
<body>
	...
	<div id=memberform>
	<span style="display:block;text-align:center;">회원수정( * 필수입력사항입니다.)</span>
	<form action="${root}/member/updateOk.do" method="post" class="form"
		...
		<div class="label">
			...
			*<input type="text" name="jumin1" value="${memberDto.jumin1}" disabled="disabled" /> -
			<input type="text" name="jumin2" value="${memberDto.jumin2}" disabled="disabled" style="margin-left:0px;"/>
		</div>
		<div class="label mar">
			<label>이메일</label>
		...
		<div class="label mar">
			<label>주소</label>
			<input type="text" name="address" value="${memberDto.address}" size="50"/>
		</div>
		<div class="label">
			<label>직업</label>
			<select name="job">
		...
		<div class="label">
			<label>메일수신 ${memberDto.mailing}</label>
		...
		<div class="label">
		    ...
		    <c:forTokens var="interest" items="${memberDto.interest}" delims=",">
		    	<script type="text/javascript">
					for (var i=0; createChkForm.interest.length; i++){
